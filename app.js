...
app.post('/get/user', (req, res) => {
  ...
  connection.query(getUserQuery, [Password,Email], (err, rows) => {
    ...
    if (rows.length > 0) {
      res.status(200).send({
        name: rows[0].Name, step_limit: rows[0].Step_Limit,
        water_limit: rows[0].Water_Limit, calories_limit: rows[0].Calories_Limit,
        carbs_limit: rows[0].Carbs_Limit, fats_limit: rows[0].Fats_Limit,
        Proteins_limit: rows[0].Proteins_Limit
      })
    }
    else {
      res.status(400).send()
...
const getMealsFromUser = async (password, email, startingdate) => {
  const getMealsQuery = `SELECT * FROM Meal_Users INNER JOIN Meals ON Meal_Users.Meal_Id = Meals.Id WHERE User_Id = ? AND Meal_Users.Date >= ? AND Meal_Users.Date <= ?`
  try {
...
app.post('/get/weekly/water', (req, res) => {
  ...
  connection.query(getUserQuery, [Password, Email], (err, users) => {
    ...
    if (users.length > 0) {
      const getWeeklyWatersQuery = `SELECT * FROM Water_Users WHERE User_Id = ? AND Date >= ? AND  Date < ?;`
      connection.query(getWeeklyWatersQuery, [users[0].Id, new Date(startingDate), new Date(new Date(startingDate).getFullYear(), new Date(startingDate).getMonth(), new Date(startingDate).getDate() + 8)], (err, rows) => {
        if (err) {
          res.status(500).send()
          throw err
        }
