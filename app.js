...
app.post('/get/weekly/meals', (req, res) => {
  ...
  connection.query(getUserQuery, [Password, Email], (err, users) => {
    ...
    if (users.length > 0) {
      const getMealsQuery = `SELECT * FROM Meal_Users INNER JOIN Meals ON Meal_Users.Meal_Id = Meals.Id WHERE User_Id = ? AND Meals.Date BETWEEN ? AND ? ORDER BY Meals.Date;`
